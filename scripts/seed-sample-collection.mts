// scripts/seed-sample-collection.mts — a collection with enough shape to exercise every state the UI has.
// Usage (bash; PowerShell: set $env:TURSO_DATABASE_URL first):
//   TURSO_DATABASE_URL=file:hitstreak.local.db npx tsx scripts/seed-sample-collection.mts --as renzo@example.com [--name "Sample Collection"]
//
// Development only. It picks real printings out of whatever catalog the local database has, so the
// cards differ per machine, and it goes through lib/collections rather than raw SQL so every row is
...
/** Priced printings spread across the value range, so the collection is not all one kind of card. */
const PICK_PRICED = `
...
/** Cost basis a little under market, so the collection shows a gain rather than a flat line. */
const paidFor = (market: number, factor: number) => Math.round(market * factor * 100) / 100;

async function main(): Promise<number> {
  const as = arg("as");
  const name = arg("name") ?? "Sample Collection";
  if (!as) {
    console.error("usage: --as <email> [--name <collection name>]");
    return 2;
  }
  const c = await db();
  ...
  if (priced.length < 6) {
    console.error("this catalog has too few priced printings to make a useful collection — run the price ingest first");
    return 1;
  }
  ...
  for (const [i, r] of priced.entries()) {
    ...
    await addItem(userId, collection.id, {
  ...
  await addItem(userId, collection.id, { printingId: Number(uncosted.id), quantity: 1, condition: "NM" });
  ...
  await addItem(userId, collection.id, { printingId: Number(partial.id), quantity: 2, condition: "MP" });
  note.push(`partly costed: ${partial.name}`);

  if (unpriced) {
    await addItem(userId, collection.id, { printingId: Number(unpriced.id), quantity: 1, condition: "NM" });
    note.push(`no market price: ${unpriced.name}`);
  } else {
    note.push("no unpriced printing in this catalog — the 'no price' state is not covered");
  }

  console.log(`seeded collection ${collection.id} "${collection.name}" for ${as}`);
  for (const n of note) console.log(`  · ${n}`);
  console.log(`  · undo: DELETE FROM collection_items WHERE collection_id = ${collection.id}; DELETE FROM collections WHERE id = ${collection.id};`);
  return 0;
}
