# Hitstreak Phase 2b — Portfolios, Search, Sets, Card Detail Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** The first real product screens on the Binder design: user portfolios ("binders") with holdings valued from `latest_prices`, a type-ahead card search, the set browser with completion tracking and tap-to-own, and a card detail page — all composed from `components/ui/` primitives only.

**Architecture:** Two new tables (`portfolios`, `collection_items`) added to the self-initializing schema, scoped by Better Auth's text `user.id`. Reads go through two data-layer modules (`lib/portfolios.ts`, `lib/catalog.ts`) that take `userId` explicitly and always join through `portfolios.user_id` (referential/ownership integrity is enforced in code, spec §5). Mutations are Next.js **server actions** in `app/(app)/portfolios/actions.ts` that call `getSession()` first. Screens are Server Components that fetch via the data layer and render primitives; the only client components are the interactive bits (add-item dialog, tap-to-own tile, search box). Money stays `REAL` dollars, formatted at display via `lib/format.ts`.
...
**Reference:** Spec §5 (data model), §7 (application surface — catalog, portfolios, card detail), §12 (UI approach). Mockups: `docs/design/Main.dc.html` (portfolio home), `Sets.dc.html`, `Card.dc.html`. Tokens/primitives: `docs/design/README.md` "Implemented as".
...
**Deferred to Phase 3 (do NOT build here):** price history charts, `portfolio_history`, share links, alerts, the nightly endpoint. `/decks` and `/alerts` get "coming soon" placeholders so the nav does not 404.
...
```
lib/schema.ts                        + portfolios, collection_items (PORTFOLIO_SCHEMA_SQL appended)
lib/portfolios.ts                    portfolio + item CRUD, holdings valuation
...
tests/portfolios.test.ts             data layer
tests/catalog.test.ts                (exists: ingest catalog tests) → new file tests/catalog-read.test.ts
app/(app)/portfolios/actions.ts      server actions: createPortfolio, renamePortfolio, deletePortfolio, addItem, updateItem, removeItem
app/(app)/portfolios/page.tsx        binder list + create
app/(app)/portfolios/[id]/page.tsx   holdings, value, add item
app/(app)/portfolios/[id]/HoldingsTable.tsx      client (edit qty / remove)
app/(app)/portfolios/[id]/AddItemDialog.tsx      client (search → printing → qty/condition/price)
app/(app)/portfolios/PortfolioForm.tsx            client (create / rename)
...
app/(app)/sets/[id]/SetGrid.tsx      client (filter pills, target-binder pill, tap-to-own)
...
### Task 1: Schema + portfolios data layer
...
### Task 4: Server actions + portfolio list page
...
export async function renamePortfolioAction(id: number, name: string) {
  const r = await withUser(async (u) => { if (!(await P.renamePortfolio(u, id, name))) throw new Error("Portfolio not found"); });
  if (r.ok) { revalidatePath("/portfolios"); revalidatePath(`/portfolios/${id}`); }
  return r;
}
export async function deletePortfolioAction(id: number) {
  const r = await withUser(async (u) => { if (!(await P.deletePortfolio(u, id))) throw new Error("Portfolio not found"); });
  if (r.ok) { revalidatePath("/portfolios"); revalidatePath("/sets"); }
  return r;
}
export async function addItemAction(portfolioId: number, input: P.AddItemInput) {
  ...
  if (r.ok) { revalidatePath(`/portfolios/${portfolioId}`); revalidatePath("/portfolios"); revalidatePath("/sets"); }
  return r;
}
export async function updateItemAction(portfolioId: number, itemId: number, patch: { quantity?: number; acquiredPrice?: number | null }) {
...
export async function removeItemAction(portfolioId: number, itemId: number) {
  ...
  if (r.ok) { revalidatePath(`/portfolios/${portfolioId}`); revalidatePath("/sets"); }
  return r;
}
```
...
### Task 5: Portfolio detail — holdings + add item
