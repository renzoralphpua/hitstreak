...
- **`scripts/import-deck.mts` does not run.** Task 8 moved `parseDecklist` / `formatDecklist` / `mergeResolved` into `lib/decks/decklist.ts`, re-exported by `lib/decks/resolve.ts`; tsx's strict-ESM path for a `.mts` entry point does not see `export *` re-exports, so the CLI throws "does not provide an export named 'mergeResolved'" at import. The same is true of `lib/decks/gap.ts` → `gap-math.ts` and `lib/history.ts` → `lib/ranges.ts`: **`scripts/*.mts` must import from the leaf module.** Product code, the bundler and `npx tsx -e` are unaffected (verified both), which is why the test suite is green. One-line fix, left to the branch review.
...
- `withUser` returns `e.message` verbatim, so a malformed direct action call can echo a raw `TypeError` to the caller. Admin-only surfaces today.
